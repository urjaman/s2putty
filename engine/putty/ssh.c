...
const static struct ssh_kex *const kex_algs[] = {
...
struct ssh_tag {
    ...
    /*
     * Text manipulation buffers for port forwarding. These are kept
     * here to simplify the state machine code in the protocol
     * funtions. We can't put these to stack under Symbian OS (too
     * big), and using local variables for the heap allocated buffers
     * would be cumbersome since we'd need to free and reallocate them
     * around every coroutine call. "One of the PuTTY source files
     * contains possibly the worst piece of C preprocessor hackery
     * I've ever seen in production code." indeed.
     */
    char *sports;
    char *dports;
    char *saddr;
...
static int do_ssh1_login(Ssh ssh, unsigned char *in, int inlen, int ispkt)
{
    ...
    fflush(stdout);
    {
	if ((statics()->flags & FLAG_INTERACTIVE) && !*ssh->cfg.username) {
	    if (statics()->ssh_get_line && !statics()->ssh_getline_pw_only) {
		if (!statics()->ssh_get_line("login as: ",
	 ...
	 * authentication.
	 */
	if (statics()->ssh_get_line) {
	    if (!statics()->ssh_get_line(s->prompt, s->password,
...
static void ssh1_protocol(Ssh ssh, unsigned char *in, int inlen, int ispkt)
{
	...
	int sport,dport,sserv,dserv;
        ...
        /* Alloc text manipulation buffers */
        sfree(ssh->sports); ssh->sports = NULL;
        ssh->sports = snewn(256, char);
        sfree(ssh->dports); ssh->dports = NULL;
        ssh->dports = snewn(256, char);
        sfree(ssh->saddr); ssh->saddr = NULL;
        ssh->saddr = snewn(256, char);
        sfree(ssh->host); ssh->host = NULL;
        ssh->host = snewn(256, char);
	...
	while (*ssh->portfwd_strptr) {
		...
		if (*ssh->portfwd_strptr == ':') {
		     ...
		     * ssh->saddr and start sports from the beginning
		     * again.
		     */
		    ssh->portfwd_strptr++;
		    ssh->sports[n] = '\0';
		    strcpy(ssh->saddr, ssh->sports);
		    n = 0;
		}
		if (n < 255) ssh->sports[n++] = *ssh->portfwd_strptr++;
	    }
	    ssh->sports[n] = 0;
	    if (type != 'D') {
		if (*ssh->portfwd_strptr == '\t')
		...
		n = 0;
		while (*ssh->portfwd_strptr && *ssh->portfwd_strptr != ':') {
		    if (n < 255) ssh->host[n++] = *ssh->portfwd_strptr++;
		}
		ssh->host[n] = 0;
		if (*ssh->portfwd_strptr == ':')
		    ssh->portfwd_strptr++;
		n = 0;
		while (*ssh->portfwd_strptr) {
		    if (n < 255) ssh->dports[n++] = *ssh->portfwd_strptr++;
		}
		ssh->dports[n] = 0;
		ssh->portfwd_strptr++;
		dport = atoi(ssh->dports);
		dserv = 0;
		if (dport == 0) {
		    dserv = 1;
		    dport = net_service_lookup(ssh->dports);
		    if (!dport) {
			logeventf(ssh, "Service lookup failed for"
				  " destination port \"%s\"", ssh->dports);
		    }
		}
	    } else {
		while (*ssh->portfwd_strptr) ssh->portfwd_strptr++;
	    ...
	    sport = atoi(ssh->sports);
	    sserv = 0;
	    if (sport == 0) {
		sserv = 1;
		sport = net_service_lookup(ssh->sports);
		if (!sport) {
		...
		char *sportdesc = dupprintf("%.*s%.*s%.*s%.*s%d%.*s",
			(int)(*ssh->saddr?strlen(ssh->saddr):0), *ssh->saddr?ssh->saddr:NULL,
			(int)(*ssh->saddr?1:0), ":",
			(int)(sserv ? strlen(ssh->sports) : 0), ssh->sports,
			sserv, "(", sport, sserv, ")");
		/* FIXME: sportdesc is kept allocated over a coroutine wait */
		assert(0);
		if (type == 'L') {
		    /* Verbose description of the destination port */
		    char *dportdesc = dupprintf("%s:%.*s%.*s%d%.*s",
			    ssh->host,
			    (int)(dserv ? strlen(ssh->dports) : 0), ssh->dports,
			    dserv, "(", dport, dserv, ")");
		    const char *err = pfd_addforward(ssh->host, dport,
						     *ssh->saddr ? ssh->saddr : NULL,
						     sport, ssh, &ssh->cfg);
		    if (err) {
		    ...
		    strcpy(pf->dhost, ssh->host);
		    pf->dport = dport;
		    if (ssh->saddr && *(ssh->saddr)) {
			logeventf(ssh,
        ...
        /* Free text manipulation buffers */
        sfree(ssh->sports); ssh->sports = NULL;
        sfree(ssh->dports); ssh->dports = NULL;
        sfree(ssh->saddr); ssh->saddr = NULL;
        sfree(ssh->host); ssh->host = NULL;
...
static void do_ssh2_authconn(Ssh ssh, unsigned char *in, int inlen, int ispkt)
{
	...
	if (s->got_username && !ssh->cfg.change_username) {
	     ...
	     * it again.
	     */
	} else if ((statics()->flags & FLAG_INTERACTIVE) &&
                   !*ssh->cfg.username) {
	    if (statics()->ssh_get_line && !statics()->ssh_getline_pw_only) {
		if (!statics()->ssh_get_line("login as: ",
	...
	while (1) {
		 ...
		 * output of (say) plink.)
		 */
		if (statics()->flags & (FLAG_VERBOSE | FLAG_INTERACTIVE)) {
	    ...
	    if (s->need_pw) {
		if (statics()->ssh_get_line) {
		    if (!statics()->ssh_get_line(s->pwprompt, s->password,
                                                 sizeof(s->password), TRUE)) {
			/*
	...
	while (*ssh->portfwd_strptr) {
		...
		if (type == 'L') {
			...
			ssh2_pkt_addbool(ssh, 1);/* want reply */
			if (*ssh->saddr)
			    ssh2_pkt_addstring(ssh, ssh->saddr);
...
static const struct telnet_special *ssh_get_specials(void *handle)
{
    ...
    Ssh ssh = (Ssh) handle;
    int i = 0;
    int num_specials = (lenof(ignore_special) + lenof(ssh2_session_specials) +
                        lenof(specials_end));
...
#define ADD_SPECIALS(name) \
    do { \
	assert((i + lenof(name)) <= num_specials); \
...
void *new_sock_channel(void *handle, Socket s)
{
    ...
    if (c) {
	c->remoteid = (unsigned) -1;	       /* to be set when open confirmed */
	c->localid = alloc_channel_id(ssh);
	c->closes = 0;

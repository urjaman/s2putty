...
int cmdline_process_param(char *p, char *value, int need_save, Config *cfg)
{
	...
	SAVEABLE(0);
	statics()->default_protocol = cfg->protocol = PROT_SSH;
	statics()->default_port = cfg->port = 22;
	return 1;
    }
    if (!strcmp(p, "-telnet")) {
	RETURN(1);
	UNAVAILABLE_IN(TOOLTYPE_FILETRANSFER | TOOLTYPE_NONNETWORK);
	SAVEABLE(0);
	statics()->default_protocol = cfg->protocol = PROT_TELNET;
	statics()->default_port = cfg->port = 23;
	return 1;
    }
    if (!strcmp(p, "-rlogin")) {
	RETURN(1);
	UNAVAILABLE_IN(TOOLTYPE_FILETRANSFER | TOOLTYPE_NONNETWORK);
	SAVEABLE(0);
	statics()->default_protocol = cfg->protocol = PROT_RLOGIN;
	statics()->default_port = cfg->port = 513;
	return 1;
    }
    if (!strcmp(p, "-raw")) {
	RETURN(1);
	UNAVAILABLE_IN(TOOLTYPE_FILETRANSFER | TOOLTYPE_NONNETWORK);
	SAVEABLE(0);
	statics()->default_protocol = cfg->protocol = PROT_RAW;
    }
    if (!strcmp(p, "-v")) {
	RETURN(1);
	...
	statics()->ssh_get_line = cmdline_get_line;
	statics()->ssh_getline_pw_only = TRUE;
    }

...
int WINAPI WinMain(HINSTANCE inst, HINSTANCE prev, LPSTR cmdline, int show)
{
    ...
    hinst = inst;
    statics()->flags = FLAG_VERBOSE | FLAG_INTERACTIVE;
	...
	statics()->default_protocol = be_default_protocol;
	/* Find the appropriate default port. */
	{
	    int i;
	    statics()->default_port = 0; /* illegal */
	    for (i = 0; backends[i].backend != NULL; i++)
		if (backends[i].protocol == statics()->default_protocol) {
